...
create index if not exists ontrail_private_users_role_index on ontrail_private.users(role);
create index if not exists ontrail_private_users_name_index on ontrail_private.users(name);
create index if not exists ontrail_private_users_email_index on ontrail_private.users(email);
...
create or replace function ontrail_private.check_role_exists() returns trigger
  ...
  as $$
begin
  if not exists (select 1 from pg_users r where r.rolname = new.role) then
    raise foreign_key_violation using message = 'Invalid role ' || new.role;
  end if;
  return new;
end
$$;
...
begin
  if (new.role <> old.role) THEN
    raise foreign_key_violation using message = 'role cannot changed after creation from ' || old.role || ' to ' || new.role;
  end if;
  return new;
end
$$;
...
begin
  ...
  execute 'create role ' || quote_ident(_role) || ' nologin'; -- because bobby tables
  perform ontrail_private.grant_permissions(_role);
  insert into ontrail_private.users (email, pass, name, role) values (
    signup.email, signup.pass, signup.name, _role
  );
